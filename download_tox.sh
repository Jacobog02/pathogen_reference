...
#datasets download virus genome taxon --inputfile ${tax_file} --annotated --refseq --include genome,annotation --filename seqs/viral.zip 


#one_acc=`tail -n +2 taxons/${a_abv}_summary.tsv | awk -F '\t' '{print $1}' `
#datasets download genome accession ${one_acc} --include genome,gff3 --filename seqs/bacterial.zip   
